...
#include "vwscapture/websocket.h"
...
socket_t WebSocket::hostname_connect(const std::string& hostname, int port)
{
...
readyStateValues WebSocket::getReadyState()
{
    return readyState;
}
...
void WebSocket::poll(int timeout)  // timeout in milliseconds
{
    ...
    while (txbuf.size()) {
        ...
        // printf("_____CHECK (Send)POLL: %d %d \n", (int)txbuf.size(), ret);
        if (false) { } // ??
        else if (ret < 0 && (socketerrno == SOCKET_EWOULDBLOCK || socketerrno == SOCKET_EAGAIN_EINPROGRESS)) {
            break;
        }
...
void WebSocket::open(string url)
{
    char host[128];
    int port;
    char path[128];
    ...
    fprintf(stderr, "Websocket: connecting: host=%s port=%d path=/%s\n", host, port, path);
    ...
    }
    {
        ...
        snprintf(line, 256, "\r\n"); ::send(sockfd, line, strlen(line), 0);
        for (i = 0; i < 2 || (i < 255 && line[i-2] != '\r' && line[i-1] != '\n'); ++i) { if (recv(sockfd, line+i, 1, 0) == 0) { return; } }
        line[i] = 0;
        if (i == 255) { fprintf(stderr, "ERROR: Got invalid status line connecting to: %s\n", url.c_str()); return; }
        if (sscanf(line, "HTTP/1.1 %d", &status) != 1 || status != 101) { fprintf(stderr, "ERROR: Got bad status connecting to %s: %s", url.c_str(), line); return; }
...
void WebSocket::dispatch(string& callable) {
    ...
    while (true) {
        ...
        if (false) { }
            ...
            if (ws.fin) {
                callable = std::string(receivedData.begin(), receivedData.end());
